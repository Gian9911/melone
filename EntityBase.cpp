...
m_type(EntityType::Base), m_referenceTile(nullptr), m_state(EntityState::Idle), m_id(0), m_collidingOnX(false),
...
void EntityBase::ApplayFriction(float l_x, float l_y) {
    if (m_velocity.x != 0){
...
void EntityBase::Update(float l_dT) {
    ...
    float friction_y = (m_speed.x*frictionValue.y)*l_dT;
    ApplayFriction(friction_x, friction_y);
