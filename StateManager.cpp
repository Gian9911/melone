...
void StateManager::Draw() {
   ...
   if (m_states.back().second->IsTransparent() && m_states.size() > 1){
       ...
       for (; itr != m_states.end(); ++itr){
           m_shared->m_wind->GetRenderindow()->setView(itr->second->GetView());
           itr->second->Draw();
       }
   }else{
...
void StateManager::SwitchTo(const StateType &l_type) {
    ...
    for (auto itr = m_states.begin(); itr != m_states.end(); ++itr){
        if (itr->first == l_type){
            ...
            tmp_state->Activate();
            m_shared->m_wind->GetRenderindow()->setView(tmp_state->GetView());
            return;
        }
    }
    ...
    m_shared->m_wind->GetRenderindow()->setView(m_states.back().second->GetView());
}
...
void StateManager::CreateState(const StateType &l_type) {
    ...
    state->m_view = m_shared->m_wind->GetRenderindow()->getDefaultView();
